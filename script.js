...
function getALLVegetableData() {
    const newArr = foodOriginalData.filter(
...
function getALLCalorieAbove100() {
    const newArr = foodOriginalData.filter(
        (item) => item.calorie > 100
    );
    console.log(newArr);
}
getALLCalorieAbove100()
//problem9

function getALLCalorieBelow100() {
    const newArr = foodOriginalData.filter(
        (item) => item.calorie < 100
    );
    console.log(newArr);
}
getALLCalorieBelow100()

//problem10

function getAllHighestProtein() {
    let newArr = foodOriginalData.map((items) => items)
    let b = newArr.sort((a, b) => b.protiens - a.protiens);
    console.log(b);
}

getAllHighestProtein()

//problem11

function getAllLowestCab() {
    let anotherArr = foodOriginalData.map((items) => items)
    let a = anotherArr.sort((a, b) => a.cab - b.cab);
    console.log(a);
}
getAllLowestCab()
